...
const PlaceholderBus = ({ onPartSelect }) => {
  const group = useRef();

  // Función genérica para manejar clic en partes
  const handleClick = (e, partName) => {
  ...
  return (
    <group ref={group} dispose={null}>
      ...
      <mesh position={[0, 1.5, 0]} onClick={(e) => handleClick(e, 'carroceria')}>
        <boxGeometry args={[2.5, 1.5, 6]} />
        <meshStandardMaterial color="#cccccc" />
      </mesh>

      {/* Parabrisas */}
      <mesh position={[0, 1.8, 3.01]} onClick={(e) => handleClick(e, 'parabrisas_delantero')}>
      ...
      {[[-1.2, 0.5, 2], [1.2, 0.5, 2], [-1.2, 0.5, -2], [1.2, 0.5, -2]].map((pos, i) => (
        <mesh key={i} position={pos} rotation={[0, 0, Math.PI / 2]} onClick={(e) => handleClick(e, `rueda_${i}`)}>
      ...
      {/* Parachoques Frontal */}
      <mesh position={[0, 0.7, 3.1]} onClick={(e) => handleClick(e, 'parachoques_frontal')}>
...
const BusViewer3D = ({ onPartSelect }) => {
  return (
    <div style={{ width: '100%', height: '400px', background: '#222', borderRadius: '10px', overflow: 'hidden' }}>
      <Canvas camera={{ position: [5, 4, 8], fov: 45 }}>
        ...
        <PlaceholderBus onPartSelect={onPartSelect} />
